...
test('reader latches the last epub selection before menu-triggered paragraph focus in web mode', async ({
  page
}) => {
  const bookUrl = '/samples/sample-book.epub';
  await page.addInitScript((key) => {
  ...
  await expect
    .poll(async () => {
      ...
      message: 'expected the EPUB reader to mount the target text before building the race reproducer selection'
    })
    .toBe(true);
  ...
  await expect
    .poll(async () => {
      ...
      message: 'expected the EPUB selection-owned popup to appear before reproducing the menu race'
    })
    .toBe(true);
...
test('reader clears the armed epub paragraph-focus guard after same-book navigation controls in web mode', async ({
  page
}) => {
  const bookUrl = '/samples/sample-book.epub';
  await page.addInitScript((key) => {
  ...
  await expect
    .poll(async () => {
      ...
      message: 'expected the EPUB reader to mount the target text before arming the paragraph-focus guard'
    })
    .toBe(true);
  ...
  await expect
    .poll(async () => {
      ...
      message: 'expected the EPUB selection-owned popup to appear before arming the route guard'
    })
    .toBe(true);
  ...
  await expect(page.getByRole('menuitem', { name: '打开段落聚焦' })).toHaveCount(0);
