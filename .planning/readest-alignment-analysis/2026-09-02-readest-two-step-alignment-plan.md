...
status. Next is C12, not started. Upstream classifications are unchanged.

Completed in [`2026-09-02-readest-high-priority-audit.md`](./2026-09-02-readest-high-priority-audit.md): all 678 high-priority pull-delta commits have a `covered`, `gap`, `partial`, or `not-applicable` decision and map to 55 remaining br1-native task IDs. Closed slices include `S1-R01` through `S1-R03`, `S2-S01` through `S2-S04`, `S2-R03A` through `S2-R03E`, `S2-R04A1` through `S2-R04A3`, `S2-R04B`, and `S2-R04C1` through `S2-R04C11`. C11A covers instant vertical input/coordinates; C11B adds native drag/animation and cancellation, completing parent `c5304cd46`. Earlier task-count changes reflect finer decomposition, not new upstream commits. C5 retains the native text-preview boundary, not full rich-media popup parity. C6 covers native internal-link destinations, not popup-internal links, PDF/TXT cues or packaged-platform acceptance. C7 rejects known-hidden rendered targets but preserves upstream navigation for unknown targets; it does not measure unrendered chapter styles. C8 retains primary annotation ownership and does not add a second popup renderer or Foliate dependency. C9 fixes existing Foliate resource owners, not fixed-layout disposal or transactional navigation failure recovery. The nested TTS obligation in `1d4b7eed8` remains partial under `S2-T03`; the nested scrolled-margin obligation in `1d8ed3fc9` remains partial under `S2-U01B`, independently of C4's background proof. [Authored-layout commit audit and acceptance map](./2026-09-05-authored-layout-commit-audit.md) records all 34 parent commits and 15 nested foliate ranges; C17 includes four independently executable IDPF substeps.

Current completed slice: `S2-R04C11B`, within its frozen native scope. X-only drag and two-phase animation preserve local duration, background and positive coordinates. Cancellation owns gesture state, frames and completion tails; four approved strict-false history guards preserve other renderers' undefined-success behavior. Locked section/iframe loading keeps admission until real completion, suppresses cancelled-load reanchors and permits explicit same-section recovery. Final4 covers 80 unique browser cases: initially 79 passed and one C9 resource-release case failed; that resource suite passed two reruns (10/10), and an ownership trace reproduced the failure as a generic pending adjacent load crossing close. It remains a documented, unfixed deferred issue, not a stable 80/80 claim. All 11 focused C11 cases, 99 helpers, 6 sibling ZIP tests, type/syntax checks and Vite build pass. Continue next with `S2-R04C12 - Ruby/furigana selection and copy semantics`; it has not started. Mixed-direction preload, full vertical-lr scrolled layout, FXL/PDF, cross-iframe key forwarding, generic pending-open/resource cancellation, navigation rollback and packaged-platform acceptance remain separate. The ledger now has 60 covered, 406 partial, 77 gap and 135 not-applicable commits, with 55 remaining primary task IDs. Each slice needs its own implementation and verification commit.
