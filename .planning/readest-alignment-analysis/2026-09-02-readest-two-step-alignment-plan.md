...
Priority follow-up completed (2026-09-06, user approved): the pending paginator
load teardown defect was repaired before C12. Final2 verifies 98 unique browser
cases, including 23 resource and 11 unchanged C11 cases, plus helpers, type/syntax
checks and direct Vite build. [Evidence and proof limits](./2026-09-06-pending-paginator-close.md).
The C11B snapshot below records the earlier failure; it is not a current unfixed
status. C12 and C13A evidence are complete. C13B1 adds new-note provenance
and C13B2a covers bookmark provenance. C13B2b library progress is complete;
freeze the remaining B2 writer slices, then B3 compatibility/restore protection
before C13C layout.

Completed in [`2026-09-02-readest-high-priority-audit.md`](./2026-09-02-readest-high-priority-audit.md): all 678 high-priority pull-delta commits have a `covered`, `gap`, `partial`, or `not-applicable` decision and map to 54 remaining br1-native task IDs. Closed slices include `S1-R01` through `S1-R03`, `S2-S01` through `S2-S04`, `S2-R03A` through `S2-R03E`, `S2-R04A1` through `S2-R04A3`, `S2-R04B`, and `S2-R04C1` through `S2-R04C12`. C11A covers instant vertical input/coordinates; C11B adds native drag/animation and cancellation, completing parent `c5304cd46`. C12 covers host-native ruby selection/copy, completing `9a05935ca` without a Foliate change. Earlier task-count changes reflect finer decomposition, not new upstream commits. C5 retains the native text-preview boundary, not full rich-media popup parity. C6 covers native internal-link destinations, not popup-internal links, PDF/TXT cues or packaged-platform acceptance. C7 rejects known-hidden rendered targets but preserves upstream navigation for unknown targets; it does not measure unrendered chapter styles. C8 retains primary annotation ownership and does not add a second popup renderer or Foliate dependency. C9 fixes existing Foliate resource owners, not fixed-layout disposal or transactional navigation failure recovery. The nested TTS obligation in `1d4b7eed8` remains partial under `S2-T03`; the nested scrolled-margin obligation in `1d8ed3fc9` remains partial under `S2-U01B`, independently of C4's background proof. [Authored-layout commit audit and acceptance map](./2026-09-05-authored-layout-commit-audit.md) records all 34 parent commits and 15 nested foliate ranges; C17 includes four independently executable IDPF substeps.

Historical C11B snapshot: X-only drag and two-phase animation preserve local duration, background and positive coordinates. Cancellation owns gesture state, frames and completion tails; four approved strict-false history guards preserve other renderers' undefined-success behavior. Locked section/iframe loading keeps admission until real completion, suppresses cancelled-load reanchors and permits explicit same-section recovery. Final4 covered 80 unique browser cases: initially 79 passed and one C9 resource-release case failed; that resource suite passed two reruns (10/10), and an ownership trace reproduced the failure as a generic pending adjacent load crossing close. That checkpoint was not a stable 80/80 claim; the subsequent pending-load repair above closes the captured defect. All 11 focused C11 cases, 99 helpers, 6 sibling ZIP tests, type/syntax checks and Vite build passed at that checkpoint.

Latest production slice: `S2-R04C13B2b`. Library progress retains its paired origin through persistence, imports, snapshot/KOReader transport and route-target projection. Imported/legacy origins stay unknown; selecting a KOReader location never borrows the local origin. Raw metadata cannot inject progress provenance, and absent/empty origins remain distinct route identities. Final4 passed 144 Node helpers, 2 Vite-loaded snapshot tests, 6 browser cases, 63 Rust tests, type/build checks and independent Terra/Sol reviews. [B1/B2a/B2b evidence and proof limits](./2026-09-06-warichu-historical-locators.md). The ledger stays at 61 covered, 405 partial, 77 gap and 135 not-applicable commits, with 54 remaining primary task IDs. These prerequisites do not advance parent coverage: historical replay and other locator families remain open. Next, freeze the remaining `S2-R04C13B2` cache/reading-asset writer slices, then complete B3 compatibility/restore write protection before C13C measured layout. Mixed-direction preload, full vertical-lr scrolled layout, FXL/PDF layout parity, cross-iframe key forwarding, generic pending-open/resource cancellation, navigation rollback and packaged-platform acceptance remain separate. Each slice needs its own implementation and verification commit.
