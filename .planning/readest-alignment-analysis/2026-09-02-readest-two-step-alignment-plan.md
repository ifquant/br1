...
status. C12 is complete; C13A is an evidence-only prerequisite and the next
production gate is C13B historical locator compatibility, before C13C layout.
...
Latest completed production slice: `S2-R04C12`. Visible ruby and original DOM/CFI/selection TTS are preserved while body and popup action/copy text excludes `rt/rp`; an exposed off-screen toolbar was fixed at its shared positioning owner. Verification passed 97 unique browser cases, 99 helpers, final type checks and direct Vite build, with Terra task and Astra whole-change source review. [C12 evidence and proof limits](./2026-09-06-ruby-selection.md). The ledger stays at 61 covered, 405 partial, 77 gap and 135 not-applicable commits, with 54 remaining primary task IDs. C13A adds only [historical locator evidence and execution gates](./2026-09-06-warichu-locator-contract.md); it does not advance parent coverage. Continue with `S2-R04C13B - Historical locator compatibility`, then C13C measured layout. Mixed-direction preload, full vertical-lr scrolled layout, FXL/PDF layout parity, cross-iframe key forwarding, generic pending-open/resource cancellation, navigation rollback and packaged-platform acceptance remain separate. Each slice needs its own implementation and verification commit.
