...
continue with C13B2 remaining writers, then B3 compatibility/restore protection
before C13C layout.
...
Latest completed production slice: `S2-R04C13B1`. New EPUB note/highlight CFIs retain rendered/pristine origin through persistence, export and merge paths; highlight toggling uses origin plus CFI, and failed exact selection generation cannot save a substitute position. Verification passed 61 unique browser cases, 113 helpers, 59 Rust tests, type checks and direct Vite build, with Terra task and Astra whole-change static reviews. [B1 evidence and proof limits](./2026-09-06-warichu-historical-locators.md). The ledger stays at 61 covered, 405 partial, 77 gap and 135 not-applicable commits, with 54 remaining primary task IDs. Neither C13A evidence nor C13B1 advances parent coverage: historical replay and other locator families remain open. Continue with `S2-R04C13B2 - Remaining locator provenance`, then B3 compatibility and restore write protection before C13C measured layout. Mixed-direction preload, full vertical-lr scrolled layout, FXL/PDF layout parity, cross-iframe key forwarding, generic pending-open/resource cancellation, navigation rollback and packaged-platform acceptance remain separate. Each slice needs its own implementation and verification commit.
