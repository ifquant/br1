...
Step 1, `S2-S01` through `S2-S04`, `S2-R03A` through `S2-R03E`, `S2-R04A1` through `S2-R04A3`, `S2-R04B`, and `S2-R04C1` through `S2-R04C7` are complete within their documented native scope. C8A adds excerpt source correspondence; C8B binds native popup selections to pristine-section CFI round trips and exact request identity through a dedicated selection channel. Parent `631cd6454` remains partial. Continue with `S2-R04C8C - Gate footnote actions and annotation persistence`, followed by C8D reverse mapping/redraw. The 59 remaining primary task IDs are unchanged; each slice needs its own implementation and verification commit.
