...
Step 1, `S2-S01` through `S2-S04`, `S2-R03A` through `S2-R03E`, `S2-R04A1` through `S2-R04A3`, `S2-R04B`, and `S2-R04C1` through `S2-R04C7` are complete within their documented native scope. C8A adds native excerpt source correspondence and restricted Range mapping only; parent `631cd6454` remains partial. Continue with `S2-R04C8B - Bind popup selections to validated source locations`, followed by C8C actions/persistence and C8D reverse mapping/redraw. The 59 remaining primary task IDs are unchanged; each slice needs its own implementation and verification commit.
