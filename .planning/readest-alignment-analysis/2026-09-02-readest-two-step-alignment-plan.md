...
and C13B2a covers bookmark provenance; continue with C13B2b library progress
and remaining writers, then B3 compatibility/restore protection
before C13C layout.
...
Latest production slice: `S2-R04C13B2a`. Bookmark identity and navigation targets retain independently paired origins through persistence and merges. CFI toggle/active state uses origin plus locator; failed bookmark loading blocks changes until a current successful retry, rather than allowing an empty list to overwrite stored records. [B1/B2a evidence and proof limits](./2026-09-06-warichu-historical-locators.md). The ledger stays at 61 covered, 405 partial, 77 gap and 135 not-applicable commits, with 54 remaining primary task IDs. These provenance prerequisites do not advance parent coverage: historical replay and other locator families remain open. Continue with `S2-R04C13B2b - Library progress provenance`, then remaining B2 cache/reading-asset writers and B3 compatibility/restore write protection before C13C measured layout. Mixed-direction preload, full vertical-lr scrolled layout, FXL/PDF layout parity, cross-iframe key forwarding, generic pending-open/resource cancellation, navigation rollback and packaged-platform acceptance remain separate. Each slice needs its own implementation and verification commit.
