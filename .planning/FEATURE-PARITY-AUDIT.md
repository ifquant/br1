...
- Focused desktop webdriver coverage now imports those same sample formats through the real `import_library_books` command, advances reading state, verifies they move from shelf into the reading workflow with persisted restore signals after returning from reader, and then reopens them through refreshed library hrefs into visible restore state【/Users/dev/workspace2/hc_apps/br1/e2e/app.e2e.ts:1324】
