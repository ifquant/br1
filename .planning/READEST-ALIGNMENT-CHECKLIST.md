...
- [x] P20-1.15 Verify EPUB focused-reading context stays human-readable on reopen
  - Outcome: this slice smoke-certifies the adjacent visible-context contract for the same real EPUB selection-owned seam: when paragraph-focused reading opens from that Foliate-owned excerpt and later reopens from the hidden same-book resume payload, the smoke first captures a real human-readable progress percentage from the EPUB footer, then requires that same percentage and a non-empty human-readable source value to appear in the overlay context on first open and reopen while the primary UI still keeps raw restore locators such as `epubcfi(...)` out of view.
  - Touches: `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
  - Verify: `pnpm -C /Users/dev/workspace2/hc_apps/br1 check` (PASS); `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --workers=1 --grep "reader reuses the exited epub selection-owned focused-reading excerpt on reopen in web mode"` (PASS); `git -C /Users/dev/workspace2/hc_apps/br1 diff --check` (PASS).
  - Done commit: 4a38cad.
  - Tutorial: `tutorials/commit/0685-verify-epub-focused-reading-context-contract.md`.
  - Notes: this remains smoke-only and visible-UI-only on purpose. It proves the overlay shows `摘录来源`/`进度` with selection-owned EPUB text on first open and reopen, that the source chip stays non-empty and human-readable, and that the exact human-readable footer percentage captured before opening focused reading is preserved in the overlay context on both opens while `epubcfi(...)` does not leak into the overlay copy. It does not certify a new restore format, any footer/overlay parity beyond that visible percentage and visible source chip, reload behavior, route refactors, or any TTS/translation/sidebar expansion.

- [x] P20-1.16 Verify EPUB hidden focused-reading resume across exit -> reload -> reopen
  - Outcome: this slice smoke-certifies the combined EPUB seam through the real Foliate selection-owned path: after paragraph-focused reading opens from a live EPUB selection, the reader exits the overlay, clears the live selection, moves progress, reloads the same book, and then manually reopens paragraph focus while still restoring the hidden same-book excerpt.
  - Touches: `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
  ...
  - Notes: this remains same-book and EPUB-only on purpose. It proves reload does not auto-open the overlay, manual reopen still wins over the missing live selection, and the reopened overlay keeps the same selection-owned excerpt plus the same visible source/progress values captured before exit. It does not claim cross-book restore, new locator formats, or any broader TTS/translation/sidebar behavior.
