...
| 2026-05-04 | Collapse sync status into per-panel timelines | this commit | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader can open sync workspace inside the notebook shell"`; `git diff --check` | folds each panel’s recent activity and last-result details into compact status timelines so current-book export and whole-library sync each read as one coherent lane |
