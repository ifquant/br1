...
| 2026-05-04 | Tighten sync workspace presentation hierarchy | this commit | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader can open sync workspace inside the notebook shell"`; `git diff --check` | collapses detached export/import/remote result cards back into the per-panel status timelines so current-book and whole-library sync each read as one coherent surface |
