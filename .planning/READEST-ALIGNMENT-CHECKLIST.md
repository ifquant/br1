...
| 2026-05-05 | Restore the selected translation archive in the correct notebook lane | pending | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader restores the selected translation ai history record for the current book in web mode"`; `git diff --check` | restores the AI workspace into the translation lane when the persisted archived selection is translation-owned and constrains the notebook to one active archived record across lookup and translation lanes |
