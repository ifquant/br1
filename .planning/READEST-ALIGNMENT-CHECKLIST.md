...
- [x] P20-1.18 Narrow the EPUB paragraph-focus latch into a one-shot route guard
  - Outcome: the EPUB paragraph-focus fallback now stages the latest live EPUB selection only long enough to arm a one-shot route guard when that selection clears, consumes the armed guard on the next focused-reading launch, and clears it again on book switches plus the shared same-book primary navigation controls (`prev`, `next`, `start`, `href`, `fraction`) so stale selections do not linger as a persistent cache.
  - Touches: `src/lib/reader/maturityMode.ts`, `src/lib/reader/readingMode.test.ts`, `src/routes/reader/+page.svelte`, `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
  - Verify: `pnpm -C /Users/dev/workspace2/hc_apps/br1 check` (PASS); `cd /Users/dev/workspace2/hc_apps/br1 && rm -rf .tmp-reader-helper-tests && pnpm exec tsc -p tsconfig.json --outDir .tmp-reader-helper-tests --noEmit false && node --test ./.tmp-reader-helper-tests/src/lib/reader/readingMode.test.js && rm -rf .tmp-reader-helper-tests` (PASS); `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --workers=1 --grep "reader latches the last epub selection before menu-triggered paragraph focus in web mode|reader clears the armed epub paragraph-focus guard after same-book navigation controls in web mode"` (PASS); `git -C /Users/dev/workspace2/hc_apps/br1 diff --check` (PASS).
  ...
  - Notes: this follow-up keeps the fix route-owned on purpose because the race still happens when route chrome steals focus from the EPUB iframe. The smoke only proves the positive menu-clear race plus one `next`-control stale-clear seam; helper assertions cover the listed `prev`, `start`, `href`, and `fraction` clear cases, and the route now funnels direct `href`/primary-navigation helpers through the same shared primary dispatcher. Hidden focused-reading reopen behavior still comes from `focusedReadingState`, not from the route guard, and this slice does not widen PDF/CBZ behavior, TTS/translation ownership, or broader reader refactors.
