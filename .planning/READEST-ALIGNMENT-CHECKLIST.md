...
| 2026-05-04 | Split reader sync activity into current-book and library lanes | this commit | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader can open sync workspace inside the notebook shell"`; `git diff --check` | adds separate recent-activity cards for current-book export and whole-library import/push/pull work, with status and timestamps, and hardens the focused smoke to wait for the reader shell before opening the sync tab |
