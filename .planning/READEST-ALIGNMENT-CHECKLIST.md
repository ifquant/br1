...
- [x] P20-1.18 Narrow the EPUB paragraph-focus latch into a one-shot route guard
  ...
  - Notes: this follow-up keeps the fix route-owned on purpose because the race still happens when route chrome steals focus from the EPUB iframe. The smoke only proves the positive menu-clear race plus one `next`-control stale-clear seam; helper assertions cover the extra `href` and `fraction` clear cases, and the route now funnels direct bookmark/search/TTS jump callers through the same shared primary dispatcher. Hidden focused-reading reopen behavior still comes from `focusedReadingState`, not from the route guard, and this slice does not widen PDF/CBZ behavior, TTS/translation ownership, or broader reader refactors.
