...
- [x] P20-1.18 Narrow the EPUB paragraph-focus latch into a one-shot route guard
  - Outcome: the EPUB paragraph-focus fallback now stages the latest live EPUB selection only long enough to arm a one-shot route guard when that selection clears, consumes the armed guard on the next focused-reading launch, and clears it again on book switches plus the shared same-book primary navigation controls (`prev`, `next`, `start`, `href`, `fraction`) so stale selections do not linger as a persistent cache. The follow-up boundary now also keeps same-book navigation from being undone by a later Foliate `selectionchange(null)` re-arm.
  - Touches: `src/lib/reader/maturityMode.ts`, `src/lib/reader/readingMode.test.ts`, `src/routes/reader/+page.svelte`, `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
  - Verify: `pnpm -C /Users/dev/workspace2/hc_apps/br1 check` (PASS); `cd /Users/dev/workspace2/hc_apps/br1 && rm -rf .tmp-reader-helper-tests && pnpm exec svelte-kit sync && pnpm exec tsc -p tsconfig.json --outDir .tmp-reader-helper-tests --noEmit false && node --test ./.tmp-reader-helper-tests/src/lib/reader/readingMode.test.js --test-name-pattern "focused-reading route keeps the menu-triggered epub clear path armed for one launch|focused-reading route suppresses delayed epub selection clears after same-book navigation|focused-reading route clears delayed-clear suppression when another book opens" && rm -rf .tmp-reader-helper-tests` (PASS); `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --grep "reader latches the last epub selection before menu-triggered paragraph focus in web mode|reader does not re-arm the epub paragraph-focus guard after same-book navigation delays the live selection clear in web mode"` (PASS); `git -C /Users/dev/workspace2/hc_apps/br1 diff --check` (PASS).
  ...
  - Notes: this follow-up keeps the fix route-owned on purpose because the race still happens when route chrome steals focus from the EPUB iframe. The route now carries an explicit sticky suppression bit so same-book navigation can keep the one-shot guard cleared even if Foliate reports the null selection slightly later. The smoke proves both the positive menu-clear race and the delayed-clear same-book navigation seam; helper assertions still cover the listed `prev`, `start`, `href`, and `fraction` clear cases plus the suppression reset when a fresh live selection or a different book arrives. Hidden focused-reading reopen behavior still comes from `focusedReadingState`, not from the route guard, and this slice does not widen PDF/CBZ behavior, TTS/translation ownership, or broader reader refactors.
