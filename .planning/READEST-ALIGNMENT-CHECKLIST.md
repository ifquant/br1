...
| 2026-05-05 | Tighten lane headers so focused AI sections read like notebook sections | pending | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader can move from the ai archive overview into one lane and back again"`; `git diff --check` | promotes the focused lookup/translation lane title and current-book record count into the lane header so the notebook section remains self-describing after the reader drills down from the overview |
