...
| 2026-05-05 | Add current-book scope summaries to the AI assistant and translation tabs | pending | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader can open the ai workspace inside the notebook shell|reader can open translation mode as a dedicated notebook tab"`; `git diff --check` | adds explicit current-book scope summaries at the top of the assistant and translation workspaces so the reader can see tab-level scope before drilling into notebook sections |
