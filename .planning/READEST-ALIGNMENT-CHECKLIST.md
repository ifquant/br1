...
- [x] P17-1.2 Extract the playback panel from the TTS workspace
  ...
  - Verify: `pnpm -C /Users/dev/workspace2/hc_apps/br1 check` (PASS); `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --workers=1 --grep "reader tts workspace exposes mature playback controls in web mode"` (PASS); `git -C /Users/dev/workspace2/hc_apps/br1 diff --check` (PASS).
  - Done commit: 7a8c7c9.
...
- [x] P20-1.14 Verify EPUB selection-owned focused reading across exit -> reopen
  - Outcome: this slice smoke-certifies a narrow EPUB-only selection-owned seam in real web reading mode: after the reader creates a real Foliate text selection, opens paragraph-focused reading from that selection-owned excerpt, exits the overlay, clears the live selection, and moves EPUB progress, a manual reopen still reuses the hidden same-book selection-owned excerpt and the source chip reads `当前选区`.
  - Touches: `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
...
- [x] P20-1.15 Verify EPUB focused-reading context stays human-readable on reopen
  - Outcome: this slice smoke-certifies the adjacent visible-context contract for the same real EPUB selection-owned seam: when paragraph-focused reading opens from that Foliate-owned excerpt and later reopens from the hidden same-book resume payload, the smoke first captures a real human-readable progress percentage from the EPUB footer, then requires that same percentage and the `当前选区` source chip to appear in the overlay context on first open and reopen while the primary UI still keeps raw restore locators such as `epubcfi(...)` out of view.
  - Touches: `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
  - Verify: `pnpm -C /Users/dev/workspace2/hc_apps/br1 check` (PASS); `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --workers=1 --grep "reader reuses the exited epub selection-owned focused-reading excerpt on reopen in web mode"` (PASS); `git -C /Users/dev/workspace2/hc_apps/br1 diff --check` (PASS).
  - Done commit: 4a38cad.
  - Tutorial: `tutorials/commit/0685-verify-epub-focused-reading-context-contract.md`.
  - Notes: this remains smoke-only and visible-UI-only on purpose. It proves the overlay shows `摘录来源`/`进度` with selection-owned EPUB text on first open and reopen, that the source chip reads `当前选区`, and that the exact human-readable footer percentage captured before opening focused reading is preserved in the overlay context on both opens while `epubcfi(...)` does not leak into the overlay copy. It does not certify a new restore format, any footer/overlay parity beyond that visible percentage and visible source chip, reload behavior, route refactors, or any TTS/translation/sidebar expansion.

- [x] P20-1.16 Verify EPUB hidden focused-reading resume across exit -> reload -> reopen
  ...
  - Done commit: 96551d7, a85d7f4, 09d0057, 7fa718c, 936dedb, e168991.
...
- [x] P20-1.17 Latch the last non-empty EPUB selection for paragraph-focus launch
  - Outcome: this slice fixes the adjacent EPUB paragraph-focus launch gap at the route boundary: the smoke evidence reproduces the header-menu focus-clear path, and the implementation guard keeps shared route-owned paragraph-focus launch on the last explicit non-empty EPUB selection when the live selection has already vanished, so the source chip stays `当前选区` instead of falling back to `当前章节正文`.
  ...
  - Done commit: 26659c4, 1a0eedc, 1f03ef6.
  - Tutorial: `tutorials/commit/0687-latch-last-epub-selection-for-paragraph-focus-launch.md`.
  - Notes: the latch stays route-owned on purpose because the observed race happens when route chrome steals focus from the EPUB iframe, not inside the pure focused-reading helper. The smoke only certifies the header-menu focus-clear path, while the implementation guard is intentionally scoped to EPUB paragraph-focus launches through the shared route handler after the live selection has already vanished. Hidden same-book focused-reading reopen semantics still come from `focusedReadingState` instead of this route latch.

- [x] P20-1.18 Narrow the EPUB paragraph-focus latch into a one-shot route guard
  - Outcome: the EPUB paragraph-focus fallback now stages the latest live EPUB selection only long enough to arm a one-shot route guard when that selection clears, consumes the armed guard on the next focused-reading launch, and clears it again on book switches plus the shared same-book primary navigation controls (`prev`, `next`, `start`, `href`, `fraction`) so stale selections do not linger as a persistent cache. The follow-up boundary now also keeps same-book EPUB navigation from being undone either by a later Foliate `selectionchange(null)` re-arm or by a stale route-owned EPUB `currentReaderSelection` surviving until header chrome steals focus.
  - Touches: `src/lib/reader/maturityMode.ts`, `src/lib/reader/readingMode.test.ts`, `src/routes/reader/+page.svelte`, `tests/e2e/library-smoke.spec.ts`, checklist/tutorial docs.
  - Verify: `pnpm -C /Users/dev/workspace2/hc_apps/br1 check` (PASS); `cd /Users/dev/workspace2/hc_apps/br1 && rm -rf .tmp-reader-helper-tests && pnpm exec svelte-kit sync && pnpm exec tsc -p tsconfig.json --outDir .tmp-reader-helper-tests --noEmit false && node --test --test-name-pattern "focused-reading route keeps the menu-triggered epub clear path armed for one launch|focused-reading route suppresses delayed epub selection clears after same-book navigation|focused-reading route clears delayed-clear suppression when another book opens|focused-reading route clears the live selection immediately on same-book navigation|focused-reading route keeps non-epub selections out of the epub-only navigation fix" ./.tmp-reader-helper-tests/src/lib/reader/readingMode.test.js && rm -rf .tmp-reader-helper-tests` (PASS); `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --grep "reader latches the last epub selection before menu-triggered paragraph focus in web mode|reader does not reuse the pre-navigation epub selection after same-book navigation in web mode|reader clears the armed epub paragraph-focus guard after same-book navigation controls in web mode"` (PASS); `git -C /Users/dev/workspace2/hc_apps/br1 diff --check` (PASS).
  - Done commit: 51f5fe5, f00343c, 2a5993e, 962f6d9, 6504607.
  - Tutorial: `tutorials/commit/0688-narrow-epub-paragraph-focus-latch-into-a-one-shot-guard.md`.
  - Notes: this follow-up keeps the fix route-owned on purpose because the race still happens when route chrome steals focus from the EPUB iframe. The route now carries an explicit sticky suppression bit so same-book EPUB navigation can keep the one-shot guard cleared even if Foliate reports the null selection slightly later, and it also drops the route-owned EPUB live selection as soon as EPUB navigation changes reading context so paragraph focus cannot reuse stale text before that null event arrives. The smoke proves both the positive menu-clear race and the narrower same-book EPUB navigation launch seam where paragraph focus opens after navigation without reusing the old selection; helper assertions cover the delayed-clear suppression itself, the listed `prev`, `next`, `start`, `href`, and `fraction` clear cases, the EPUB-only immediate-clear boundary, plus the suppression reset when a fresh live selection or a different book arrives. Hidden focused-reading reopen behavior still comes from `focusedReadingState`, not from the route guard, and this slice does not widen PDF/CBZ behavior, TTS/translation ownership, or broader reader refactors.
...
| 2026-05-09 | Keep the in-reader TTS mini bar visible during translated waiting states | `86b0c85` | `pnpm check`; `cd /Users/dev/workspace2/hc_apps/br1 && rm -rf .tmp-tts-tests && pnpm exec svelte-kit sync && pnpm exec tsc -p tsconfig.json --outDir .tmp-tts-tests --noEmit false && perl -0pi -e "s#from './tts';#from './tts.js';#g; s#from './ttsRuntime';#from './ttsRuntime.js';#g" ./.tmp-tts-tests/src/lib/reader/tts.test.js ./.tmp-tts-tests/src/lib/reader/tts.js ./.tmp-tts-tests/src/lib/reader/ttsRuntime.test.js && node --test ./.tmp-tts-tests/src/lib/reader/tts.test.js ./.tmp-tts-tests/src/lib/reader/ttsRuntime.test.js`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader uses visible plain-text excerpts as the source tts target in web mode|reader can open tts mode as a dedicated notebook tab"`; `git diff --check` | keeps the in-reader mini playback bar visible while translated TTS is waiting on a selected live or archived translation source, and replaces the generic empty-target fallback with source-tied waiting copy |
