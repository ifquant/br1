...
| 2026-05-04 | Add explicit empty-state copy to sync timelines | this commit | `pnpm check`; `CI=1 pnpm test:e2e tests/e2e/library-smoke.spec.ts --grep "reader can open sync workspace inside the notebook shell"`; `git diff --check` | adds readable initial-state copy for both timelines so first-open sync workspaces explain that no current-book export or whole-library sync has been performed yet |
