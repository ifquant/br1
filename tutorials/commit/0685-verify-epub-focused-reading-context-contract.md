...
- asserted the overlay shows readable context labels for `摘录来源` and `进度`, plus the selection-owned `当前选区` source
- reused the real EPUB reader progress to prove a human-readable percentage is surfaced in the overlay context
...
- no claim that the smoke proves footer/overlay string parity beyond the visible progress percentage it asserts
