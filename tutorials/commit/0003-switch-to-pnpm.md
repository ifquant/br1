...
- `pnpm-lock.yaml` 是 `pnpm` 的真实依赖决议来源
- `package-lock.json` 是 `npm` 的真实依赖决议来源

这两个锁文件不能长期并存。
一旦并存，不同的人可能会用不同工具把依赖树改成两个版本，最后出现“代码没变，但你机器能跑、我机器不能跑”的问题。
...
这次能把安装真正跑通，还有一个很细但很重要的点：`package.json` 里原来有一条无效的自引用依赖 `"br1": "link:"`。

这种依赖声明的问题在于：

- 它不是正常的 registry 依赖
- 也不是一个清晰的 workspace 依赖
- 安装工具看到它时，很容易产生不确定行为

一个实用判断标准是：

- 如果一个包就是当前项目本身，就不应该再把自己写成自己的依赖
- workspace 依赖要写成清晰的 workspace 形式，而不是模糊占位
