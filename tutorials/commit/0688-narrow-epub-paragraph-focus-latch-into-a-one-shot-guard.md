...
The remaining holes were both EPUB timing seams around same-book navigation. First, same-book navigation could clear the route guard, but a slightly later `selectionchange(null)` from Foliate could rebuild that guard from `previousSelection` and bring the stale excerpt back. Second, the EPUB route still held onto `currentReaderSelection` until Foliate sent that later null event, so a menu-triggered paragraph-focus launch could reuse the old excerpt even when the one-shot guard itself stayed cleared. This slice closes both gaps with explicit EPUB route-owned boundaries instead of timing guesses.
...
- added an EPUB-only route-owned selection-clear boundary for reading-context control requests so same-book navigation drops stale `currentReaderSelection` immediately instead of waiting for a later Foliate null event
...
- added focused web smoke proving both the positive menu-clear race and the narrower same-book EPUB navigation launch seam where paragraph focus opens after navigation without reusing the old selection

## Verification

- `pnpm -C /Users/dev/workspace2/hc_apps/br1 check`
- `cd /Users/dev/workspace2/hc_apps/br1 && rm -rf .tmp-reader-helper-tests && pnpm exec svelte-kit sync && pnpm exec tsc -p tsconfig.json --outDir .tmp-reader-helper-tests --noEmit false && node --test --test-name-pattern "focused-reading route keeps the menu-triggered epub clear path armed for one launch|focused-reading route suppresses delayed epub selection clears after same-book navigation|focused-reading route clears delayed-clear suppression when another book opens|focused-reading route clears the live selection immediately on same-book navigation" ./.tmp-reader-helper-tests/src/lib/reader/readingMode.test.js && rm -rf .tmp-reader-helper-tests`
- `pnpm -C /Users/dev/workspace2/hc_apps/br1 exec playwright test tests/e2e/library-smoke.spec.ts --grep "reader latches the last epub selection before menu-triggered paragraph focus in web mode|reader does not reuse the pre-navigation epub selection after same-book navigation in web mode"`
