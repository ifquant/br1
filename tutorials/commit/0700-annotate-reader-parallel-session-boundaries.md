...
- the secondary pane starts empty on route rebuild and reuses the primary open target when it is opened as an in-memory companion
...
- annotated `parallel.ts` so the pane invariants around route-derived source identity, opening control requests, open targets, preview/progress snapshots, and mount state read as one coordinated session model
...
- no attempt to restore two independent durable reader sessions or to clone the primary pane's live preview/progress state into secondary
