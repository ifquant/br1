...
本次提交把 `FB2 / MOBI / AZW3 / CBZ` 的证据，从“能导入并打开”推进到“能完成一次真实的 library -> reader -> library 往返，并留下可恢复的阅读进度信号”。
...
4. 主动向前翻一步，确保不是只停留在初始页
5. 点击 `Go to library`
6. 等待磁盘上的 `library.json` 出现更新后的 `lastOpenedAt`，并且带出 `progressFraction` 或 `progressLocation`
7. 刷新 library 页面，确认这本书已经进入 `continue reading` 或 `recent reading`，并且新的 reader href 也带有 `fraction` 或 `location`

这样锁住的就不只是 reader 打开，而是整个产品工作流里的“可恢复进度”契约。
...
- 多格式的 metadata / annotation 一致性还没完全对齐
- 还没有补 `open with br1` 的 OS 级集成证据

所以这次是把证据推进到“多格式进入正式阅读工作流，并留下可恢复进度”，不是宣告整个格式主线已经收口。
