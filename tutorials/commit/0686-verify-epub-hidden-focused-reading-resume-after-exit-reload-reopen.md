...
- after exit, the smoke clears the live EPUB selection, moves reading progress, reloads the same book, and confirms the overlay stays closed, the live Foliate DOM selection stays empty, and the visible reader progress stays away from the original excerpt until the reader manually reopens paragraph focus from the menu
- made the final assertions prove the hidden same-book payload survives reload: manual reopen still restores the same selection-owned excerpt, the same human-readable source chip, and the same progress percentage captured before exit even though the live viewport progress had stayed away from that original excerpt first
